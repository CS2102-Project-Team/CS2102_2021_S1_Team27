...
async function insertBid(
  ...
  const price = await db.query('SELECT price * ($4::DATE - $3::DATE + 1) AS totalprice FROM looksafter WHERE ctaker=$1 AND ptype=$2', [ctaker, ptype, startdate, enddate]).rows[0].totalprice;
