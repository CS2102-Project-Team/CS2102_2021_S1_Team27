...
async function promotePartime(username) {
  const { rows } = await db.query('BEGIN; UPDATE caretakers SET fulltime=true, maxpets=5 WHERE username = $1; INSERT INTO looksafter(ctaker, price, ptype) SELECT $1, 0, ptype FROM pettypes; SELECT update_price(ptype) FROM pettypes; COMMIT;', [username]);
  return rows;
}
